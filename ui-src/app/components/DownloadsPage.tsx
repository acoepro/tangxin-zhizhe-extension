...
function taskTone(task: DownloadTask) {
  if (task.stage === "complete" || task.stage === "ready") {
    return { label: downloadStageLabel(task.stage), color: "bg-emerald-100 text-emerald-600", icon: <CheckCircle size={11} /> };
  }
  if (task.stage === "error") {
    return { label: "失败", color: "bg-rose-100 text-rose-600", icon: <XCircle size={11} /> };
  }
  if (["playlist", "segments", "segment"].includes(String(task.stage || ""))) {
    return { label: downloadStageLabel(task.stage), color: "bg-amber-100 text-amber-600", icon: <Download size={11} className="animate-bounce" /> };
  }
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={() => onAction("open-download-folder")}
          className="flex items-center gap-1 rounded-xl bg-gradient-to-r from-sky-400 to-blue-500 px-3 py-2 text-xs text-white shadow-sm transition-transform active:scale-95"
        >
          <FolderOpen size={13} /> 下载目录
        </button>
        <button
          onClick={() => onAction("refresh-downloads")}
          className="flex items-center gap-1 rounded-xl bg-gradient-to-r from-purple-400 to-violet-500 px-3 py-2 text-xs text-white shadow-sm transition-transform active:scale-95"
        >
          <RefreshCw size={13} /> 刷新
        </button>
        <button
          onClick={() => onAction("save-downloads")}
          className="flex items-center gap-1 rounded-xl bg-gradient-to-r from-emerald-400 to-teal-500 px-3 py-2 text-xs text-white shadow-sm transition-transform active:scale-95"
        >
          <Save size={13} /> 保存记录
        </button>
        <button
          onClick={() => onAction("clear-downloads")}
          className="ml-auto flex items-center gap-1 rounded-xl border border-rose-200 px-3 py-2 text-xs text-rose-500 transition-transform active:scale-95"
        >
          <Trash2 size={13} /> 清空
        </button>
      </div>
      ...
      <div className="space-y-3">
        {tasks.length ? tasks.map((task) => {
          ...
          return (
            <div
              key={task.taskId || task.movieId || task.url}
              className="space-y-2 rounded-2xl border border-pink-100 bg-white p-3 shadow-sm"
            >
              {/* 标题行 */}
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-[10px] text-purple-300">
                    {task.movieId ? `视频 ${task.movieId}` : task.taskId || "视频任务"}
                  </p>
                </div>
              ...
              {(task.error || task.transmuxError) && (
                <div className="flex items-start gap-1.5 rounded-xl bg-rose-50 p-2">
                  ...
                  <p className="text-[10px] text-rose-600 break-all">
                    {task.error || `MP4 转封装失败，TS 文件已保留：${task.transmuxError}`}
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-1.5 pt-0.5">
                <button
                  ...
                  className="flex flex-1 items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-pink-400 to-purple-500 py-1.5 text-[11px] font-medium text-white shadow-sm transition-transform active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Save size={12} /> 保存到设备
                </button>
                {/* 复制下载链接（仅有 URL 时显示） */}
                {task.url && (
                  <button
                    onClick={() => onAction("copy-download-url", { taskId: task.taskId || "" })}
                    className="flex items-center gap-1 rounded-xl border border-purple-200 px-2.5 py-1.5 text-[11px] text-purple-400 hover:bg-purple-50 transition-colors"
                    title="复制下载链接"
                  >
                    <Copy size={11} />
                  </button>
                ...
                <button
                  onClick={() => onAction("remove-download-task", { taskId: task.taskId || "", movieId: task.movieId || "" })}
                  className="flex items-center gap-1 rounded-xl border border-rose-200 px-2.5 py-1.5 text-[11px] text-rose-400 hover:bg-rose-50 transition-colors"
                  title="删除任务"
                >
                  <Trash2 size={11} />
                </button>
