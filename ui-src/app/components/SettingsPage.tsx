...
// Worker 健康检查：直接 fetch /v1/health 端点
...
export function SettingsPage({ state, onAction }: Props) {
  ...
  const checkWorkerHealth = async () => {
    ...
    if (!url) {
      setPingStatus({ ok: false, text: "请先在账号池页面配置 Worker URL" });
      return;
    }
    setPinging(true);
    setPingStatus(null);
    ...
    <div className="space-y-4 p-4">
      ...
      <div className="space-y-3 rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
        <h3 className="flex items-center gap-1.5 text-sm font-bold text-purple-700">
          <Radio size={14} className="text-sky-400" /> Worker 连接检测
        </h3>
        <p className="text-xs text-purple-400">
          检测 Worker 端点 <code className="rounded bg-purple-50 px-1 py-0.5 font-mono text-[10px] text-purple-600">/v1/health</code> 的响应情况。
        </p>
        {/* 状态提示行 */}
        {pingStatus && (
        ...
        {state.remote?.baseUrl && (
          <p className="truncate text-[10px] text-purple-300 font-mono">{state.remote.baseUrl}</p>
        )}
        <button
          onClick={checkWorkerHealth}
          disabled={pinging}
          className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-sky-400 to-blue-500 py-2 text-xs font-medium text-white shadow-sm transition-all active:scale-95 disabled:opacity-70"
        >
          {pinging
            ? <><RefreshCw size={13} className="animate-spin" /> 检测中…</>
            : <><Radio size={13} /> 检测 Worker 连接</>}
        </button>
      </div>
      ...
      <div className="rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
        <h3 className="mb-2 flex items-center gap-1.5 text-sm font-bold text-purple-700">
          <Sparkles size={14} className="text-pink-400" /> 展示覆盖
        </h3>
        <p className="mb-3 text-xs text-purple-400">
          {state.displayPatchApplied
            ? "展示覆盖已应用，VIP 永久有效、余额 999、永久尤物圈已生效。"
            : "尚未应用展示覆盖，点击下方按钮立即应用。"}
        </p>
        <button
          onClick={() => onAction("apply")}
          className={`flex w-full items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-medium text-white shadow-sm transition-all active:scale-95 ${
            state.displayPatchApplied
      ...
      <div className="space-y-3 rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
        ...
        <p className="text-xs text-purple-400">
          {updateAvailable
            ? `发现新版本：${state.repositoryUpdate?.remote?.version || "远程版本"}`
            : "当前版本 v2.2.0 已是最新。插件会在启动时静默检查远程版本清单。"}
        </p>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => onAction("download-latest")}
            className="flex flex-1 items-center justify-center gap-1 rounded-xl border border-purple-200 py-2 text-xs font-medium text-purple-500 transition-transform active:scale-95"
          >
      ...
      <div className="space-y-3 rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
        ...
        <p className="text-xs text-purple-400">覆盖安装后如出现旧账号、旧播放详情或旧配置残留，可在此选择清除内容。</p>
            ...
            <label key={item} className="flex cursor-pointer items-center gap-2 select-none">
              <div
                ...
                className={`flex h-4 w-4 cursor-pointer items-center justify-center rounded-full border-2 transition-all ${
                  cacheChecked[index]
                    ? "border-transparent bg-gradient-to-br from-pink-400 to-purple-500"
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-rose-400 to-pink-500 py-2 text-xs font-medium text-white shadow-sm transition-transform active:scale-95"
        >
      ...
      <div className="rounded-2xl border border-pink-100 bg-white p-4 shadow-sm">
        ...
        <p className="mb-3 text-xs text-purple-400">
          糖心志者 v2.2.0 是一个 Chrome Manifest V3 浏览器插件，提供账号池管理、展示覆盖、播放资源获取、视频下载、版本检查等功能。
        </p>
        <button
          onClick={() => onAction("about")}
          className="flex w-full items-center justify-center gap-1 rounded-xl border border-purple-200 py-2 text-xs font-medium text-purple-500 transition-transform active:scale-95"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl bg-white p-5 shadow-2xl">
            ...
            <p className="mb-4 text-xs text-purple-400">
              {updateAvailable
                ? `远程版本 ${state.repositoryUpdate?.remote?.version || "未知"}，点击下载最新版获取更新包。`
                : "当前版本 v2.2.0 已是最新版，无需更新。"}
            </p>
            <button
              onClick={() => setShowUpdateModal(false)}
              className="w-full rounded-xl bg-gradient-to-r from-pink-400 to-purple-500 py-2 text-sm font-medium text-white shadow-md"
            >
              好的
            </button>
          </div>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl bg-white p-5 shadow-2xl">
            ...
            <p className="mb-4 text-xs text-purple-400">
              清除后需要重新同步账号池，建议操作完成后刷新当前页面。
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowClearConfirm(false)}
                className="flex-1 rounded-xl border border-pink-200 py-2 text-sm font-medium text-purple-500"
              >
                取消
              </button>
              <button
                onClick={() => { onAction("clear-cache"); setShowClearConfirm(false); }}
                className="flex-1 rounded-xl bg-gradient-to-r from-rose-400 to-pink-500 py-2 text-sm font-medium text-white shadow-md"
              >
                确认清除
              </button>
            </div>
