...
import { CheckCircle, Cloud, Coins, Crown, Edit2, Eye, EyeOff, HardDrive, Heart, Plus, RefreshCw, Trash2, Upload, X, XCircle } from "lucide-react";
import type { AccountItem, BridgeState } from "../types";
import { accountAvailable, accountName, accountRights, accountStats, accountStatusLabel, isCloudAccount, visibleAccounts } from "../helpers";
...
export function AccountsPage({ state, onAction }: Props) {
  ...
  const renderAccount = (account: AccountItem) => {
    ...
    return (
      <div key={account.id || accountName(account)} className={`rounded-2xl border bg-white p-3 shadow-sm ${selected ? "border-pink-300 ring-1 ring-pink-200" : ok ? "border-pink-100" : "border-rose-100 opacity-70"}`}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white ${ok ? "bg-gradient-to-br from-pink-400 to-purple-500" : "bg-gray-300"}`}>
              {accountName(account).slice(0, 1)}
            </div>
            <div>
              <p className="text-xs font-semibold text-purple-800">{accountName(account)}</p>
              <div className="mt-0.5 flex flex-wrap gap-1">
                {selected && <span className="rounded-full bg-pink-100 px-2 py-0.5 text-[10px] text-pink-600">已选中</span>}
                <span className={`rounded-full px-2 py-0.5 text-[10px] ${ok ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}>
        ...
        <div className="ml-10 mt-2 flex flex-wrap gap-3 text-[11px] text-purple-400">
          <span className="flex items-center gap-0.5"><Crown size={11} className={rights.vip ? "text-amber-400" : "text-gray-300"} /> VIP</span>
          <span className="flex items-center gap-0.5"><Heart size={11} className={rights.dark ? "text-pink-400" : "text-gray-300"} /> 尤物圈</span>
          <span className="flex items-center gap-0.5"><Coins size={11} className="text-amber-400" /> {rights.coins || 0} 金币</span>
        </div>

        {!cloud && (
          <div className="mt-2 flex items-center justify-end gap-1.5">
            <button onClick={() => onAction("select-account", { accountId: account.id || "" })} className="rounded-full bg-purple-50 px-2 py-1 text-[10px] text-purple-500">
              {selected ? "已选择" : "选择"}
            </button>
            <button onClick={() => onAction("upload-local-account-remote", { accountId: account.id || "" })} className="rounded-full p-1.5 text-sky-400 hover:bg-sky-50"><Upload size={12} /></button>
            <button onClick={() => onAction("edit-account", { accountId: account.id || "" })} className="rounded-full p-1.5 text-purple-400 hover:bg-purple-50"><Edit2 size={12} /></button>
            <button onClick={() => onAction("remove-account", { accountId: account.id || "" })} className="rounded-full p-1.5 text-rose-400 hover:bg-rose-50"><Trash2 size={12} /></button>
          </div>
        )}
      </div>
    );
